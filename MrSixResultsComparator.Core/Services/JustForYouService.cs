...
public class JustForYouService : ISearchService
{
    private readonly AppConfiguration _config;

    public JustForYouService(AppConfiguration config)
    {
        _config = config;
    }
        ...
        bool enableExplain = false)
    {
        ...
        if (enableExplain)
        {
            ...
            Log.Debug("Explain tracking enabled for JustForYou on {ServerName}", pinnedToServerName);
        }
        ...
        try
        {
            Log.Debug("Executing JustForYou on {ServerName} for CallId: {CallId}", pinnedToServerName, searcher.CallId);
            response = MrSIXProxyV2.SearchesV5.Recommended.Execute(args);
            Log.Debug("JustForYou completed on {ServerName} for CallId: {CallId}. Result count: {ResultCount}", 
                pinnedToServerName, searcher.CallId, response?.Results?.Count ?? 0);
        }
        catch (Exception ex)
        {
            Log.Error(ex, "JustForYou failed on {ServerName} for CallId: {CallId}", pinnedToServerName, searcher.CallId);
            throw;
        }
