...
public class LitBatchService : ISearchService
{
    private readonly AppConfiguration _config;

    public LitBatchService(AppConfiguration config)
    {
        _config = config;
    }
        ...
        bool enableExplain = false)
    {
        ...
        try
        {
            Log.Debug("Executing HighlightBatch on {ServerName} for CallId: {CallId}", pinnedToServerName, searcher.CallId);
            response = MrSIXProxyV2.SearchesV5.HighlightBatch.ExecuteBatch(args);
            Log.Debug("HighlightBatch completed on {ServerName} for CallId: {CallId}. Result count: {ResultCount}", 
                pinnedToServerName, searcher.CallId, response?.Results?.Count ?? 0);
        }
        catch (Exception ex)
        {
            Log.Error(ex, "HighlightBatch failed on {ServerName} for CallId: {CallId}", pinnedToServerName, searcher.CallId);
            throw;
        }
