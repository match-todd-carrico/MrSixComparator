...
public class TwoWayService : ISearchService
{
    private readonly AppConfiguration _config;

    public TwoWayService(AppConfiguration config)
    {
        _config = config;
    }
        ...
        bool enableExplain = false)
    {
        ...
        if (enableExplain)
        {
            ...
            Log.Debug("Explain tracking enabled for TwoWay on {ServerName}", pinnedToServerName);
        }
        ...
        try
        {
            Log.Debug("Executing TwoWay on {ServerName} for CallId: {CallId}", pinnedToServerName, searcher.CallId);
            response = MrSIXProxyV2.SearchesV5.TwoWay.Execute(args);
            Log.Debug("TwoWay completed on {ServerName} for CallId: {CallId}. Result count: {ResultCount}", 
                pinnedToServerName, searcher.CallId, response?.Results?.Count ?? 0);
        }
        catch (Exception ex)
        {
            Log.Error(ex, "TwoWay failed on {ServerName} for CallId: {CallId}", pinnedToServerName, searcher.CallId);
            throw;
        }
